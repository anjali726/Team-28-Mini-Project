...
const updateUserDetails = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.patch(`http://localhost:5000/api/users/updateProfile`, user, config)
...
const getUserDetails = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5000/api/users/${userInfo.id}`, config)
...
const updateUserRole = (role, email) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.patch('http://localhost:5000/api/users/updateUserRole', { userRole: role, email: email }, config)
...
const getWorkers = (excludedRoles) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5000/api/users/admin/getUsers?excludedRoles=${excludedRoles.join('&excludedRoles=')}`, config)
    dispatch({
